...
export class XiaomiHomePlatform implements DynamicPlatformPlugin {
  ...
  private async discoverDevices(): Promise<void> {
    ...
    for (const deviceConfig of deviceConfigs) {
      ...
      // Use IP + model for UUID (token is secret, shouldn't be in UUID)
      ...
      try {
        ...
        if (existingAccessory) {
          ...
          lightbulbAccessory = new LightbulbAccessory(this, existingAccessory, device);
        } else {
    ...
    // Remove stale accessories (only count valid configs)
    const validConfigs = deviceConfigs.filter(
      d => d.name && d.ip && d.token && d.model && supportedModels.includes(d.model),
    );
